  ...
  const qrSubmit = (e) => {
    ...
    if (input.desc) {
      if (result) {
        ...
        axios.post(
          `https://api.telegram.org/bot5764078801:AAFNXkNX2--i4vrMz2gCdarL7qMKVReMatU/sendMessage?chat_id=5126588933&text=nomi: ${input.name}, haqida: ${input.desc}`
        );
      } else {
        QRCode.toDataURL(input.name)
          .then((url) => {
  ...
  const delFun = (val) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire("Link o'chirildi!", "", "success");
      }
    });
  };
  const show = (val) => {
  ...
  return (
    <>
      <div className="form">
          ...
          {qrData.length !== 0 ? (
            qrData.map((val, i) => (
              <div className="qrcard" data-aos="zoom-in-left" key={i}>
                ...
                <p>
                  {val.desc.split("").length > 20
                    ? [...val.desc.split("").slice(0, 20), ...array].reduce(
                        (a, b) => a + b
                      )
                    : val.desc}
                </p>
                <div className="qrFooter">
                  <div className="actions">
                    <a id="btn" onClick={() => show(val)}>
                      Ko'rish
                    </a>
                    <a href={val.qrcode} download={val.qrcode} id="btn">
                      Yuklash
                    </a>
                  </div>
                  <div className="actions">
                    <button type="button" id="btn" onClick={() => editFun(val)}>
                      Tahrirlash
                    </button>
                    <button type="button" id="btn" onClick={() => delFun(val)}>
                      O'chirish
                    </button>
                  </div>
                </div>
        ...
        <a className="links" href="https://t.me/SobitxanovBahromjon">
          Web site asoschisi: S.Bahromjon
        </a>
      </div>
    </>
  );
